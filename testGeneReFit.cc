...
if (argc>2 && (strcmp(argv[2],"w") == 0) ){
  ...
  std::cout<<" Setting Wrong tagged analysis"<<std::endl;
  std::cout<<"====================================="<<endl;
  sprintf(OutFileName,"testGeneReFit-2016-Q2Bin-%d-Bins-%d-%d-%d-BernDeg-%d-%d-%d-wrongTagged.root", Q2Bin,xCosLHBin,xCosKHBin,xPhiHBin,maxDegree1,maxDegree2,maxDegree3); 
  ...
  std::cout<<"======================================="<<endl;
  sprintf(OutFileName,"testGeneReFit-2016-Q2Bin-%d-Bins-%d-%d-%d-BernDeg-%d-%d-%d.root", Q2Bin,xCosLHBin,xCosKHBin,xPhiHBin,maxDegree1,maxDegree2,maxDegree3); 
...
//     			 parLis.emplace_back(varName,varName, parIni,-100.0,100.0);
  ...
  std::fstream *parListInput = new std::fstream(ListGenValues,std::ifstream::in);
...
//       P8p.setVal(atof (mappa["P8p"].c_str() ) );
   ...
   RooAbsReal::defaultIntegratorConfig()->setEpsAbs(SetIntPrecision) ;
   RooAbsReal::defaultIntegratorConfig()->setEpsRel(SetIntPrecision) ;
